...
def write_stamp(stamp_path):
    stamp_content = "consistent\n"
    if os.path.exists(stamp_path):
        with open(stamp_path, "r", encoding="utf-8") as stamp_file:
            if stamp_file.read() == stamp_content:
                return
    ...
    temporary_path = "{}.tmp".format(stamp_path)
    with open(temporary_path, "w", encoding="utf-8") as stamp_file:
        stamp_file.write(stamp_content)

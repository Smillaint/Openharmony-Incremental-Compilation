...
def get_latest_input_mtime(input_idl_list, idl_tool_path):
    input_paths = list(input_idl_list)
    input_paths.extend([
        os.path.abspath(__file__),
        os.path.join(idl_tool_path, "idl"),
    ])
    mtimes = []
    for input_path in input_paths:
        if os.path.isfile(input_path):
            mtimes.append(os.stat(input_path).st_mtime_ns)
    if not mtimes:
        raise FileNotFoundError("IDL action has no existing input files")
    return max(mtimes)


def set_output_mtime(output_paths, mtime_ns):
    for output_path in output_paths:
        if not os.path.isfile(output_path):
            continue
        os.utime(output_path, ns=(mtime_ns, mtime_ns))
...
def idl_gen_interface(input_arguments):
    ...
    input_mtime = get_latest_input_mtime(
        src_idl_list, input_arguments.idl_tool_path
    )
    ...
    for src_idl in src_idl_list:
        ...
        if ret != 0:
            return ret
    set_output_mtime(dst_file_list, input_mtime)
    return 0
